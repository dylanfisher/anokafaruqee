...
// Add headers to disable Dreampress' varnish cache on home page
add_action( 'wp', 'sandbox_add_header_xua' );
function sandbox_add_header_xua() {
	if(is_front_page()){
...
add_image_size( 'anoka-painting-thumb', 150, 9999, false ); // Custom Image - Name, Width, Height, Hard Crop boolean
add_image_size( 'anoka-painting-thumb-installations', 195, 9999, false );
add_image_size( 'anoka-painting-thumb-landscape', 9999, 165, false );
add_image_size( 'anoka-painting-lightbox-desk-medium', 9999, 760, false );
...
/* Disable automatic paragraphs in content editor */
...
function tuts_mcekit_editor_style($url) {
    ...
    $url .= trailingslashit( get_stylesheet_directory_uri() ) . '/style-tinymce.css';
...
function tuts_mce_before_init( $settings ) {
	...
	$style_formats = array(
		array(
			'title' => 'Page title (indent)',
			'block' => 'h2',
			'classes' => 'page-title-indent'
			),
		array(
			'title' => 'Page title',
			'block' => 'h2',
...
function tuts_mcekit_editor_enqueue() {
  $StyleUrl = get_stylesheet_directory_uri().'/style-tinymce.css'; // Customstyle.css is relative to the current file
