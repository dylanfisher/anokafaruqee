...
foreach( $myposts as $post ) :	setup_postdata($post); ?>
	<?php if(get_field('paintings')): ?>
		<?php while(has_sub_field('paintings')): ?>
					<div class="painting-container" data-date="<?php the_sub_field('year'); ?>" data-size="<?php the_sub_field('size'); ?>"  <?php if (get_sub_field('serial_number') != "") { ?>data-serial="<?php the_sub_field("serial_number"); ?>" <?php } ?> <?php if (get_sub_field('image_splash') != "") { ?>data-image-detail="<?php $image_src = wp_get_attachment_image_src(get_sub_field("image_splash"),full,false); echo $image_src[0]; ?>" <?php } ?>>
...
$row_count = count($images);
$i = rand(0, $row_count - 1);
 
$oneImage = $images[$i]["image_splash"]; 
?>
	<div id="splash" style="display: none;">
		<?php echo'<img id="splash-image" src="'.wp_get_attachment_url( $oneImage, 'thumbnail' ).'"/>';	?>
	</div>
</body>
</html>